...
// ================= NOVA ROTA: AUTORIZAR TERMO DE CONSENTIMENTO =================
app.post('/clt/consult/authorize', async (req, res) => {
  try {
    ...
    const r = await axios.post(
      `https://bff.v8sistema.com/private-consignment/consult/${consult_id}/authorize`,
      {}, // body vazio
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    res.json({
      success: true,
      consult_id,
      status: r.data?.status || 'TERMO_AUTORIZADO'
    })

  } catch (e) {
    console.log('AUTHORIZE ERROR:', e.response?.data)
    res.status(400).json(e.response?.data || { erro: true })
  }
})
